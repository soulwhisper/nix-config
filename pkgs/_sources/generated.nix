# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  caddy-core = {
    pname = "caddy-core";
    version = "v2.10.2";
    src = fetchFromGitHub {
      owner = "caddyserver";
      repo = "caddy";
      rev = "v2.10.2";
      fetchSubmodules = false;
      sha256 = "sha256-KvikafRYPFZ0xCXqDdji1rxlkThEDEOHycK8GP5e8vk=";
    };
  };
  caddy-plugin-cloudflare = {
    pname = "caddy-plugin-cloudflare";
    version = "2fc25ee62f40fe21b240f83ab2fb6e2be6dbb953";
    src = fetchFromGitHub {
      owner = "caddy-dns";
      repo = "cloudflare";
      rev = "2fc25ee62f40fe21b240f83ab2fb6e2be6dbb953";
      fetchSubmodules = false;
      sha256 = "sha256-EMRdRwQLHJXVvnLe1v6WrC9BU2Aawu8mQUpz4NPm1F0=";
    };
    date = "2025-10-22";
  };
  geo-custom = {
    pname = "geo-custom";
    version = "6519b38c2459fb11aeca0bd0d927b9c4cccc0b85";
    src = fetchFromGitHub {
      owner = "soulwhisper";
      repo = "geo";
      rev = "6519b38c2459fb11aeca0bd0d927b9c4cccc0b85";
      fetchSubmodules = false;
      sha256 = "sha256-O2AmnWN/fKu0dEuS3tO5AW5FSiAnjoVlRxCqbqZQ7NQ=";
    };
    date = "2025-11-19";
  };
  hass-sgcc = {
    pname = "hass-sgcc";
    version = "v1.6.8";
    src = fetchFromGitHub {
      owner = "ARC-MX";
      repo = "sgcc_electricity_new";
      rev = "v1.6.8";
      fetchSubmodules = false;
      sha256 = "sha256-UdlFI+txh4GRldjbUUBRPfJsd95Dv/GaB6te07vQe0Y=";
    };
  };
  kubecolor-catppuccin = {
    pname = "kubecolor-catppuccin";
    version = "1d4c2888f7de077e1a837a914a1824873d16762d";
    src = fetchFromGitHub {
      owner = "vkhitrin";
      repo = "kubecolor-catppuccin";
      rev = "1d4c2888f7de077e1a837a914a1824873d16762d";
      fetchSubmodules = false;
      sha256 = "sha256-gTneUh6yMcH6dVKrH00G61a+apasu9tiMyYjvNdOiOw=";
    };
    date = "2024-05-24";
  };
  kubectl-switch = {
    pname = "kubectl-switch";
    version = "v2.3.2";
    src = fetchFromGitHub {
      owner = "mirceanton";
      repo = "kubectl-switch";
      rev = "v2.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-YfdQbjP+8UkeYCG0HSWMfXCayO6/ffEv9pT/hQ29sgI=";
    };
  };
  mosdns-custom = {
    pname = "mosdns-custom";
    version = "ab137f2b42142610ece97dd73610458794f15aa1";
    src = fetchFromGitHub {
      owner = "yyysuo";
      repo = "mosdns";
      rev = "ab137f2b42142610ece97dd73610458794f15aa1";
      fetchSubmodules = false;
      sha256 = "sha256-RTQQ2aCbTQihUYiiSD267IpKDTjJ/Iyiu9seoKNr0bI=";
    };
    date = "2025-09-12";
  };
  rime-moqi-yinxing = {
    pname = "rime-moqi-yinxing";
    version = "eb661b5d8d0fa22221466e05536954dbeaff8e56";
    src = fetchFromGitHub {
      owner = "gaboolic";
      repo = "rime-shuangpin-fuzhuma";
      rev = "eb661b5d8d0fa22221466e05536954dbeaff8e56";
      fetchSubmodules = false;
      sha256 = "sha256-uV0U5eiWwQHP24b6qoQZrfps1Ar9dnFMuwyoOwuLrdo=";
    };
    date = "2025-10-18";
  };
  singbox-custom = {
    pname = "singbox-custom";
    version = "94a8e7dca9946e30b2ea85e2061aa13b20485a68";
    src = fetchFromGitHub {
      owner = "CHIZI-0618";
      repo = "sing-box";
      rev = "94a8e7dca9946e30b2ea85e2061aa13b20485a68";
      fetchSubmodules = false;
      sha256 = "sha256-qOe8ErNSSMeOuyVw5jRXFmIfKB90kayp7hl10L4gAA4=";
    };
    date = "2025-10-31";
  };
  talos-api = {
    pname = "talos-api";
    version = "v1.0.11";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "discovery-service";
      rev = "v1.0.11";
      fetchSubmodules = false;
      sha256 = "sha256-IM+YRMEDDTWuYeYclIJhsqLmYSR1HZ6dXguc9sXmoDw=";
    };
  };
  talosctl = {
    pname = "talosctl";
    version = "v1.10.8";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "talos";
      rev = "v1.10.8";
      fetchSubmodules = false;
      sha256 = "sha256-WON2jQi8so3qZDsXVavGgSqUGBXmAYSYTdqo1o6+olk=";
    };
  };
  zotregistry = {
    pname = "zotregistry";
    version = "v2.1.10";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zot";
      rev = "v2.1.10";
      fetchSubmodules = false;
      sha256 = "sha256-Evf556J2j+qCI4OtFT40yi3fQiZ0ubOTcCL8vsGHKb8=";
    };
  };
  zotregistry-ui = {
    pname = "zotregistry-ui";
    version = "04fac87321b090b6a2137db6dfb61d612b9f2f6f";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zui";
      rev = "04fac87321b090b6a2137db6dfb61d612b9f2f6f";
      fetchSubmodules = false;
      sha256 = "sha256-wOKkYsJXqhyvBJM6nwuy8S5T+4YEyqUPuiVDBIMwTaI=";
    };
    date = "2025-11-18";
  };
}
