# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  caddy-core = {
    pname = "caddy-core";
    version = "v2.11.2";
    src = fetchFromGitHub {
      owner = "caddyserver";
      repo = "caddy";
      rev = "v2.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-QoGq8+lhaSQuC1VwIYE8h8N/ZC1ozfmIwmsIPk29Jos=";
    };
  };
  caddy-plugin-cloudflare = {
    pname = "caddy-plugin-cloudflare";
    version = "a8737d095ad5a48ca031cea6ab704057dbc2d250";
    src = fetchFromGitHub {
      owner = "caddy-dns";
      repo = "cloudflare";
      rev = "a8737d095ad5a48ca031cea6ab704057dbc2d250";
      fetchSubmodules = false;
      sha256 = "sha256-0Csi6WmyoGj7bXeo2Lrnwr0SCoV6c/niymtOp5DdiT4=";
    };
    date = "2026-03-23";
  };
  hass-sgcc = {
    pname = "hass-sgcc";
    version = "v1.7.3";
    src = fetchFromGitHub {
      owner = "ARC-MX";
      repo = "sgcc_electricity_new";
      rev = "v1.7.3";
      fetchSubmodules = false;
      sha256 = "sha256-ORGQb9TCb/D3p7B7aJjFA4Xc18v0hAg7d63IWuB2dx0=";
    };
  };
  kubecolor-catppuccin = {
    pname = "kubecolor-catppuccin";
    version = "1d4c2888f7de077e1a837a914a1824873d16762d";
    src = fetchFromGitHub {
      owner = "vkhitrin";
      repo = "kubecolor-catppuccin";
      rev = "1d4c2888f7de077e1a837a914a1824873d16762d";
      fetchSubmodules = false;
      sha256 = "sha256-gTneUh6yMcH6dVKrH00G61a+apasu9tiMyYjvNdOiOw=";
    };
    date = "2024-05-24";
  };
  kubectl-switch = {
    pname = "kubectl-switch";
    version = "v2.4.8";
    src = fetchFromGitHub {
      owner = "mirceanton";
      repo = "kubectl-switch";
      rev = "v2.4.8";
      fetchSubmodules = false;
      sha256 = "sha256-Nv03pvl0QQJ07bcZ+P4wAAU64XQNsSLPGKYSCsg3VS4=";
    };
  };
  rime-moqi-yinxing = {
    pname = "rime-moqi-yinxing";
    version = "4be1cbaa736f955e1ceeb536f72d1226ea079a96";
    src = fetchFromGitHub {
      owner = "gaboolic";
      repo = "rime-shuangpin-fuzhuma";
      rev = "4be1cbaa736f955e1ceeb536f72d1226ea079a96";
      fetchSubmodules = false;
      sha256 = "sha256-EQLnYxcS3yyyzCwmTlBj1QLzgTSiBV0j/KflVjvI2hY=";
    };
    date = "2026-04-13";
  };
  talos-api = {
    pname = "talos-api";
    version = "v1.0.17";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "discovery-service";
      rev = "v1.0.17";
      fetchSubmodules = false;
      sha256 = "sha256-hW7N3vKqvWlFHizOT6rlzNZyG6q7h0z5Fv8zavH5AQ4=";
    };
  };
  zotregistry = {
    pname = "zotregistry";
    version = "v2.1.15";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zot";
      rev = "v2.1.15";
      fetchSubmodules = false;
      sha256 = "sha256-PhPhlifLU0kOGPqH1kqKxikt+DtJO+MWEv1w6/7sZ6E=";
    };
  };
  zotregistry-ui = {
    pname = "zotregistry-ui";
    version = "1c8e5ef68b2898703f0a937de67ac60aff3e356b";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zui";
      rev = "1c8e5ef68b2898703f0a937de67ac60aff3e356b";
      fetchSubmodules = false;
      sha256 = "sha256-Kmn/YQz9NroV5oiGvad09s8XDLmDdRCO9o7zWuZZGVE=";
    };
    date = "2026-04-05";
  };
}
