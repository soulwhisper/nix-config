# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  caddy-core = {
    pname = "caddy-core";
    version = "v2.11.2";
    src = fetchFromGitHub {
      owner = "caddyserver";
      repo = "caddy";
      rev = "v2.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-QoGq8+lhaSQuC1VwIYE8h8N/ZC1ozfmIwmsIPk29Jos=";
    };
  };
  caddy-plugin-cloudflare = {
    pname = "caddy-plugin-cloudflare";
    version = "a8737d095ad5a48ca031cea6ab704057dbc2d250";
    src = fetchFromGitHub {
      owner = "caddy-dns";
      repo = "cloudflare";
      rev = "a8737d095ad5a48ca031cea6ab704057dbc2d250";
      fetchSubmodules = false;
      sha256 = "sha256-0Csi6WmyoGj7bXeo2Lrnwr0SCoV6c/niymtOp5DdiT4=";
    };
    date = "2026-03-23";
  };
  hass-sgcc = {
    pname = "hass-sgcc";
    version = "v1.7.3";
    src = fetchFromGitHub {
      owner = "ARC-MX";
      repo = "sgcc_electricity_new";
      rev = "v1.7.3";
      fetchSubmodules = false;
      sha256 = "sha256-ORGQb9TCb/D3p7B7aJjFA4Xc18v0hAg7d63IWuB2dx0=";
    };
  };
  kubecolor-catppuccin = {
    pname = "kubecolor-catppuccin";
    version = "1d4c2888f7de077e1a837a914a1824873d16762d";
    src = fetchFromGitHub {
      owner = "vkhitrin";
      repo = "kubecolor-catppuccin";
      rev = "1d4c2888f7de077e1a837a914a1824873d16762d";
      fetchSubmodules = false;
      sha256 = "sha256-gTneUh6yMcH6dVKrH00G61a+apasu9tiMyYjvNdOiOw=";
    };
    date = "2024-05-24";
  };
  kubectl-switch = {
    pname = "kubectl-switch";
    version = "v2.4.8";
    src = fetchFromGitHub {
      owner = "mirceanton";
      repo = "kubectl-switch";
      rev = "v2.4.8";
      fetchSubmodules = false;
      sha256 = "sha256-Nv03pvl0QQJ07bcZ+P4wAAU64XQNsSLPGKYSCsg3VS4=";
    };
  };
  rime-moqi-yinxing = {
    pname = "rime-moqi-yinxing";
    version = "72e1bfe5b8235305edca37e18ec3ed381b848107";
    src = fetchFromGitHub {
      owner = "gaboolic";
      repo = "rime-shuangpin-fuzhuma";
      rev = "72e1bfe5b8235305edca37e18ec3ed381b848107";
      fetchSubmodules = false;
      sha256 = "sha256-AHr6ObCgWN5yniv519AI7YMhascZRC5JpLuVTo3T8tg=";
    };
    date = "2026-04-21";
  };
  talos-api = {
    pname = "talos-api";
    version = "v1.0.17";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "discovery-service";
      rev = "v1.0.17";
      fetchSubmodules = false;
      sha256 = "sha256-hW7N3vKqvWlFHizOT6rlzNZyG6q7h0z5Fv8zavH5AQ4=";
    };
  };
  zotregistry = {
    pname = "zotregistry";
    version = "v2.1.16";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zot";
      rev = "v2.1.16";
      fetchSubmodules = false;
      sha256 = "sha256-eEyaV0PZjqYIWOgof6xejK+6TK5Ubat778ovjGQzNzk=";
    };
  };
  zotregistry-ui = {
    pname = "zotregistry-ui";
    version = "93334201a195dbe64c3bd1fcc6483b651c729456";
    src = fetchFromGitHub {
      owner = "project-zot";
      repo = "zui";
      rev = "93334201a195dbe64c3bd1fcc6483b651c729456";
      fetchSubmodules = false;
      sha256 = "sha256-5WeXPecAr4katn+bpnVZuZCzWNiR7GIuvbgkhyWmzns=";
    };
    date = "2026-04-16";
  };
}
