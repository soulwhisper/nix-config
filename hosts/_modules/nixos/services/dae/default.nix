...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    # :: conflict with daed, mihomo/singbox
    # :: dae should be isolate with other dns services
    ...
    systemd.services.dae = {
      ...
      serviceConfig = {
        PIDFile = "/run/dae.pid";
        ExecStartPre = "${pkgs.unstable.dae}/bin/dae validate --config /var/lib/dae/config.dae";
        ExecStart = "${pkgs.unstable.dae}/bin/dae run --disable-timestamp --config /var/lib/dae/config.dae";
    ...
    systemd.timers.update-dae-subs = {
      description = "Update DAE Subscription";
      timerConfig = {
        OnBootSec = "15min";
        OnUnitActiveSec = "12h";
    ...
    systemd.services.update-dae-subs = {
      ...
      wants = ["network-online.target"];
      after = ["network-online.target"];
      path = [pkgs.unstable.dae pkgs.curl];
      preStart = ''
