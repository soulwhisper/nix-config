  ...
  nftablesConfig = ''
    table inet tproxy {
      ...
      chain mangle-prerouting {
        ...
        ip daddr @local_ipv4 meta l4proto udp ct direction original goto tproxy-backend
        ip6 daddr @local_ipv6 meta l4proto udp ct direction original goto tproxy-backend
      }

      chain mangle-output {
        type route hook output priority mangle; policy accept;
        ip daddr @router_ipv4 meta l4proto udp ct direction original goto tproxy-mark
        ip6 daddr @router_ipv6 meta l4proto udp ct direction original goto tproxy-mark
      }
    ...
    systemd.services.custom-routing = {
      ...
      script = ''
        ...
        ${builtins.concatStringsSep "\n" (map (route:
          "${pkgs.iproute2}/bin/ip route add ${route} || true"
        ) ipv4Routes)}

        ${builtins.concatStringsSep "\n" (map (route:
          "${pkgs.iproute2}/bin/ip -6 route add ${route} || true"
        ) ipv6Routes)}
