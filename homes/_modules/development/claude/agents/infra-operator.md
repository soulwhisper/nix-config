...
The `permissions.ask` list in settings already prompts the user on
`kubectl apply`, `kubectl delete`, `helm upgrade`, `helm uninstall`,
`talosctl reset`, `talosctl upgrade`, `nixos-rebuild`, `darwin-rebuild`,
`nix-collect-garbage`, `git push`, `sudo`, and `rm -rf`. Treat that as the
...
- Never assume `kubectl` and `talosctl` config files exist — they're
  read-denied by default in `settings.json`. If you need them, ask the user
  to grant access for this session.

## When you are not the right tool

If the task is purely a code change with no runtime side effect (writing a
new Nix module, editing a manifest in git, refactoring a script), hand it
to the planner or just edit directly. Reserve this agent for the moments
